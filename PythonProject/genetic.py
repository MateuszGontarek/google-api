...
mutation_rate = 0.4
DistanceList = []
API_KEY = "AIzaSyB54EuM0Lgs4bWoBgKwaK0O4x-7u60ZrfM"

client = googlemaps.Client(API_KEY)

def compute_city_distance_coordinates(city_a,city_b, cities_dict):
    a = cities_dict[city_a]
    b = cities_dict[city_b]
...
def fitness_eval(city_list, cities_dict, n_cities):
    ...
    for i in range(n_cities-1):
        a = city_list[i]
        b = city_list[i+1]
        total += compute_city_distance_coordinates(a,b, cities_dict)
    return total
...
def progenitor_selection(population_set,fitnes_list):
    ...
    progenitor_list_a = population_set[np.random.choice(list(range(len(population_set))), len(population_set),p=prob_list, replace=True)]
    progenitor_list_b = population_set[np.random.choice(list(range(len(population_set))), len(population_set),p=prob_list, replace=True)]
